...
class MLP(nn.Module):
    ...
    def forward(self, idx, targets=None):
        ...
        net_logits = self.mlp(x)

        # if we are given some desired targets also calculate the loss
        net_loss = None
        if targets is not None:
            net_loss = F.cross_entropy(net_logits.view(-1, net_logits.size(-1)),
                                   targets.view(-1), ignore_index=-1)

        return net_logits, net_loss
...
class Bigram(nn.Module):
    ...
    def __init__(self, config):
        super().__init__()
        # Trainable probability table
        self.logits = nn.Parameter(torch.zeros((config.vocab_size, config.vocab_size)))

    def get_block_size(self):
        """ Returns 1, model only needs one previous token to make a prediction """
        return 1

    def forward(self, idx, targets=None):
        """ forward pass -- is just a table lookup """
        net_logits = self.logits[idx]
...
def generate(model, idx, max_new_tokens, temperature=1.0, do_sample=False, top_k=None):
    ...
    for _ in range(max_new_tokens):
        ...
        # Raw output of the model's forward pass
        logits, _ = model(idx_cond)

        # Pluck the logits at the final step and scale by desired temperature (randomness)
        ...
        # Either sample from the p-dist or highest probability
        if do_sample:
...
def evaluate(model, dataset, batch_size=50, max_batches=None):
    ...
    for i, eval_batch in enumerate(loader):
        eval_batch = [t.to(args.device) for t in eval_batch]
...
class CharDataset(Dataset):
    ...
    def contains(self, word):
        """ check if a word is in the dataset """
        return word in self.words

    def get_vocab_size(self):
        """ number of possible characters in the dataset + special 0 token """
        return len(self.chars) + 1

    def get_output_length(self):
        """ length of the output sequence, including <START> token """
        return self.max_word_length + 1

    def encode(self, word):
        """ encode a word into a tensor of integers """
        ix = torch.tensor([self.stoi[w] for w in word], dtype=torch.long)
        return ix

    def decode(self, ix):
        """ decode a tensor of integers into a word """
        word = ''.join(self.itos[i] for i in ix)
        return word

    def __getitem__(self, idx):
        """ return a pair of input and target tensors for a given example index """
        word = self.words[idx]
        ix = self.encode(word)
...
def create_datasets(input_file):
    """ create training and test datasets from file """
    # preprocessing of the input text file
    with open(input_file, 'r', encoding='utf-8') as file:
        data = file.read()

    # Parse lines as words and trim
    words = data.splitlines()
    words = [w.strip() for w in words]
    words = [w for w in words if w]

    # Determine tokens and max word length
    ...
    # Partition the input data into a training and the test sets
    test_set_size = min(1000, int(len(words) * 0.1))
    rp = torch.randperm(len(words)).tolist()
    train_words = [words[i] for i in rp[:-test_set_size]]
    test_words = [words[i] for i in rp[-test_set_size:]]
    print(f"split up the dataset into {len(train_words)} \
          training examples and {len(test_words)} test examples")
...
if __name__ == '__main__':
    ...
    # Sample from the model and quit
    if args.sample_only:
    ...
    while True:
        ...
        # Get  batch, move to device, and unpack it to input and target
