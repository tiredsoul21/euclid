...
def get_batch(split):
    ...
    data_ref = train_data if split == 'train' else val_data
    ix = torch.randint(len(data_ref) - BLOCK_SIZE, (BATCH_SIZE,))
    x = torch.stack([data_ref[i:i+BLOCK_SIZE] for i in ix])
    y = torch.stack([data_ref[i+1:i+BLOCK_SIZE+1] for i in ix])
    return x, y
...
if __name__ == "__main__":
    ...
    print(f"Data loaded with {len(dataset)} characters")

    tokens = sorted(list(set(dataset)))
    token_count = len(tokens)
    print(f"Token Count: {token_count}")

    # Create a mapping of tokens to integers
    token_to_int = dict((c, i) for i, c in enumerate(tokens))
    int_to_token = dict((i, c) for i, c in enumerate(tokens))
    def encode(s):
        """ Encode a string to a NumPy array of integers """
        return np.array([token_to_int[c] for c in s])
    def decode(l):
        """ Decode a NumPy array of integers to a string """
        return ''.join([int_to_token[i] for i in l])
...
with open('/home/derrick/repo/euclid/data/tinyshakespear.txt', 'r', encoding='utf-8') as f:
    text = f.read()
